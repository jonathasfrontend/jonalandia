...
async function searchGuild(interaction) {
    ...
    try {
        ...
        Logger.info(`Informações do Servidor ${guild.name} consultadas com sucesso por ${interaction.user.tag}`);
    } catch (error) {
        Logger.error(`Erro ao consultar informações do servidor: ${error}`);
