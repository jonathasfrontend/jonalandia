    ...
    try {
        if (commandName === 'addchannels') {
            ...
            if (subCommand === 'todos') {
                ...
                info.info(`Foram adicionados ${textChannels.size} canais de texto ao banco de dados.`);
                ...
                await ChannelModel.create(channelData).catch(err => {
                    if (err.code === 11000) {
                        interaction.editReply("Este canai já está registrado.");

                        const logChannel = client.channels.cache.get(process.env.CHANNEL_ID_LOGS_ERRO_BOT);
                        logChannel.send(`Este canai já está registrado.`);
                    }
                    return;
                });
                ...
                info.info(`O canal <#${specificChannel.id}> foi adicionado ao banco de dados.`);
            }
            
        }

    } catch (error) {
        erro.error(`Erro ao registrar canais.`, error);
